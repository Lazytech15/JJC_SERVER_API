...
app.get("/api/items/summary", (req, res) => {
  const sql = `SELECT COUNT(*) as total_items, SUM(CASE WHEN item_status = 'In Stock' THEN 1 ELSE 0 END) as in_stock, SUM(CASE WHEN item_status = 'Low In Stock' THEN 1 ELSE 0 END) as low_stock, SUM(CASE WHEN item_status = 'Out Of Stock' THEN 1 ELSE 0 END) as out_of_stock, SUM(balance) as total_quantity, SUM(cost) as total_value, AVG(price_per_unit) as avg_price_per_unit FROM itemsdb`
  ...
  db.get(sql, [], (err, row) => {
    ...
    res.json({
      ...
      data: {
        ...
        total_value: Number.parseFloat(row.total_value || 0).toFixed(2),
        avg_price_per_unit: Number.parseFloat(row.avg_price_per_unit || 0).toFixed(2),
      },
    })
  })
})
...
app.get("/api/employees/search/:query", (req, res) => {
  ...
  db.all(
    `SELECT * FROM emp_list WHERE first_name LIKE ? OR last_name LIKE ? OR middle_name LIKE ? OR username LIKE ? ORDER BY last_name, first_name`,
...
app.post("/api/employees", (req, res) => {
  ...
  db.run(
    `INSERT INTO emp_list (uid, last_name, first_name, middle_name, username, access_level, password_salt, password_hash, tfa_salt, tfa_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [uid, last_name, first_name, middle_name, username, access_level, password_salt, password_hash, tfa_salt, tfa_hash],
    (err) => {
      if (err) {
        res.status(500).json({ error: err.message })
        return
...
app.put("/api/employees/:uid", (req, res) => {
  ...
  db.run(
    `UPDATE emp_list SET last_name = ?, first_name = ?, middle_name = ?, username = ?, access_level = ?, password_salt = ?, password_hash = ?, tfa_salt = ?, tfa_hash = ? WHERE uid = ?`,
    [last_name, first_name, middle_name, username, access_level, password_salt, password_hash, tfa_salt, tfa_hash, uid],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message })
        return
...
app.put("/api/employees/:uid/password", (req, res) => {
  ...
  db.run(
    `UPDATE emp_list SET password_salt = ?, password_hash = ? WHERE uid = ?`,
...
app.put("/api/employees/:uid/tfa", (req, res) => {
  ...
  db.run(`UPDATE emp_list SET tfa_salt = ?, tfa_hash = ? WHERE uid = ?`, [tfa_salt, tfa_hash, uid], function (err) {
    if (err) {
      res.status(500).json({ error: err.message })
      return
    }
...
app.get("/api/access-levels", (req, res) => {
  db.all(
    "SELECT DISTINCT access_level FROM emp_list WHERE access_level IS NOT NULL ORDER BY access_level",
    (err, rows) => {
      if (err) {
        res.status(500).json({ error: err.message })
        return
